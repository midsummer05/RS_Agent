...
## 2026-09-05：按开发文档恢复四阶段 LLM 规划后的复验

上段结论仅适用于旧版“自由连续参数”Planner，不能代表当前实现。现已恢复预处理、解译、后处理、QA 四个受约束规划点，并把每个工具的参数限制为 Tool Registry 的有限档案；报告阶段保持确定性。完整复验产物见 [HTML 报告](../evaluation-output/phase4-/20260905T092111.076859Z/evaluation.html)、[JSON](../evaluation-output/phase4-/20260905T092111.076859Z/evaluation.json) 与 [审计](../evaluation-output/phase4-/20260905T092111.076859Z/audit.json)。

| 指标 | 规则 | 受约束 LLM |
| --- | ---: | ---: |
| 正式案例 | 24 | 24 |
| 完成率 | 83.33% | 95.83% |
| 平均 IoU | 0.300853 | 0.328561 |
| 平均 F1 | 0.368467 | 0.409777 |
| LLM HTTP 200 / 合法规划点 | 0 / 0 | 96 / 96 |
| 规则回退 / 重复完成步骤 | 0 / 0 | 0 / 0 |
| 平均时延 | 0.89 秒 | 57.70 秒 |

光学 IoU 从 0.365657 升至 0.421074；SAR 两侧均为 0.236049。审计确认 48 份任务证据、96 个唯一服务商响应 ID、331 个校验通过的产物，以及证据中不存在凭据。此结果表明有限参数档案能改善本批光学案例的自动完成与分割结果，但不构成跨数据集或生产精度结论。
