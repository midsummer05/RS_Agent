...
LLM 在预处理、解译、后处理和 QA 四个阶段做工具匹配和参数规划，不接收影像像素或人工真值；报告阶段固定规则。参数不是自由连续值：每个工具只能选择 Tool Registry 中有限、审查过的参数档案。每个案例运行使用隔离 SQLite，避免前一案例经验记忆污染；非法计划修复一次后回退规则路由。

当前每个传感器/阶段的工具选择空间仍较小，因此 100% 工具路由准确率不能证明复杂路由能力；应同时看参数档案、非法输出/回退、成本与时延。SAR 暗目标、光学云影等仍会导致明显错误。
...
QA 召回率的固定异常案例现覆盖空掩膜、主导掩膜、无效几何和无效输入；运行 `.venv/Scripts/python.exe scripts/evaluate_qa.py` 会输出独立 CSV/JSON。当前案例为合成控制样本，不能替代真实多源异常数据上的 QA 泛化结论。尚无矢量边界精度独立评测或独立恢复压力评测；不得从此批量运行推导这些能力达标。没有训练、没有证明跨数据集泛化，也没有将公开可下载等同于自由再分发授权。
